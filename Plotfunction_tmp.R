...
locateChr <- function(y, Title, subTitle){
	colText = 'grey40'
	colLines = 'grey80'
	...
	y = max(yRange)
	abline(h = 0, lty = 3)
	abline(v = cumLen[1:23], col = colLines, lty = 2)
	text(0, y, labels = "chr", cex = 0.75, col = colText)
	text(cumLen[1]/2, y, labels = 1, cex = 0.75, col = colText)
	for(i in 2:length(cumCentr)){
		x <- (hg19$length[i]/2 + cumLen[i-1])
		text(x, y, labels = i, cex = 0.75, col = colText)
		}
	}
...
locateProbes <- function(genomicPos, LogRatio){
	...
	points(g, LR, pch = 19, cex = 0.1, col = rgb(0.8, 0.8, 0.8, 0.5))
	}
...
locateSegments <- function(genomicPos, Segments, thresh){
	...
	points(g, s, pch = 19, cex = 0.25, col = ifelse(s>thresh, 'blue', ifelse(s < -thresh , 'red3', 'grey50')))
	}
