...
.getData <- function(synId){
  ...
  status <- ifelse(pars$dLRs<=.1, 'Excellent (dLRs < 0.1)',
                ifelse(pars$dLRs>.1 & pars$dLRs<=.2, 'Good (0.1 < dLRs < 0.2)',
                       ifelse(pars$dLRs>.2 & pars$dLRs<=.3, 'Poor (0.2 < dLRs < 0.3)', 'Bad (dLRs > .3)')))
...
shinyServer(function(input, output) {
  ...
  output$Id <- renderText({paste(Input$SampleId, Input$synId, sep = ' - ')})
 	output$dlrs <- renderText({sprintf('dLRs: %s', Input$dlrs)})
 	output$mad <- renderText({sprintf('MAD: %s', Input$MAD)})
 	output$status <- renderText({sprintf('QC status: %s', Input$status)})
