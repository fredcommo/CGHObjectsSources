...
shinyServer(function(input, output) {
  ...
  # createTable is the reactive function called by renderTable()
 	...
 	createPlot <- reactive({
     currentInput <- gsub('(.)*-', '', input$Samp)
 	  if(!exists('currentId') | (exists('currentId') && currentId != currentInput)){
       cat('new data\n')
       #Remove(shinyData, gPos, l2r, segTable, currentId, currentSample)
       .getData(currentInput)
 	      }
 	  cat(currentSample, '\t', currentId, '\n')
 	  output$Id <- renderText(paste(currentSample, currentInput, sep = ' - '))
 	  
    Up = input$gain; Lo = input$loss # gain = 15%, lost = 10%
    ...
    .locateChr(1.5) # yvalue: what heigh to write
 	  if(Gene$current != 'NONE' & Gene$current != ''){
  		tmp <- try(.geneOfInt(segTable, Gene$current), silent = TRUE)
