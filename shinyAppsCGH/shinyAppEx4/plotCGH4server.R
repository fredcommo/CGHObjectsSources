...
.addSegments <- function(tmp, Up, Lo, lossCol, normCol, gainCol){
...
shinyServer(function(input, output) {
  ...
  # createTable is the reactive function called by renderTable()
    ...
    normCol <- 'grey60'
  	...
  	segCols <- ifelse(segTable$seg.med <= Lo, lossCol, ifelse(segTable$seg.med >= Up, gainCol, normCol))
  	...
  	if(gene != 'NONE' & gene != ''){
  		...
  		if(class(tmp) != 'try-error')
  		  .addSegments(tmp, Up, Lo, lossCol, 'grey30', gainCol)
  	  }
	})
